...
import { RepositoryConflictError, RepositoryNotFoundError, type ConversationRepository } from "./repository"
...
export class SqliteConversationRepository implements ConversationRepository {
  ...
  appendMessage(input: AppendMessageInput): Message { return this.appendTransaction(input) }
  private readonly appendTransaction = (input: AppendMessageInput): Message => this.db.transaction(() => this.appendWithinTransaction(input)).immediate()
  private appendWithinTransaction(input: AppendMessageInput): Message {
    if (input.clientKey) { const found = this.db.query<Row, [string,string]>("SELECT * FROM messages WHERE conversation_id=? AND client_key=?").get(input.conversationId, input.clientKey); if (found) return message(found) }
