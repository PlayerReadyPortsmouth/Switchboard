...
export interface IndexEntry { path: string; scope: string; vector: number[] }
...
export class VectorIndex {
  ...
  set(path: string, scope: string, vector: number[]): void {
    this.entries.set(path, { path, scope, vector })
    this.persist()
  }
  ...
  /** Top-`limit` entries within `scopes`, ranked by cosine to `query`. */
  search(query: number[], scopes: string[], limit: number): SearchHit[] {
    const wanted = new Set(scopes)
    return [...this.entries.values()]
      .filter((e) => wanted.has(e.scope))
  ...
  private load(): void {
    if (!this.file) return
    try {
      const obj = JSON.parse(readFileSync(this.file, "utf8")) as Record<string, { scope: string; vector: number[] }>
      for (const [path, v] of Object.entries(obj)) this.entries.set(path, { path, ...v })
    } catch {}
  }
  private persist(): void {
    if (!this.file) return
    const obj: Record<string, { scope: string; vector: number[] }> = {}
    for (const e of this.entries.values()) obj[e.path] = { scope: e.scope, vector: e.vector }
