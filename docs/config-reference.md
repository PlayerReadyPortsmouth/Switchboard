...
| `webCards` | object | **absent = off** | `{ enabled, dbFile?, identityMap?, maxHistory? }` — interactive cards on the web surface. Gives every agent card a canonical, persisted representation (`<stateDir>/webcards.sqlite`) that the web transcript renders and rehydrates via `GET /api/conversations/:id/cards`, and opens `POST /api/conversations/:id/interactions` so a web click reaches the owning agent through the same `sendInteraction` frame a Discord click produces. An `update_card` becomes revision N+1 of the SAME card (latest state shown in place, anchored at first post); `maxHistory` (default `20`) bounds the retained trail of superseded states, `0` keeps none. **`identityMap` is the security-critical key**: every authorisation gate (base allowlist, `approvals.approvers`, `deployApproverUserId`, `approverOnly` gated actions) is keyed on a **Discord snowflake**, but a web caller presents an email. This map (`{"someone@example.com": "1234..."}`, keys matched case-insensitively, values used verbatim) is the only bridge, and exists so the web path reuses the existing gates unchanged instead of growing a parallel rule set. **An email with no entry is rejected outright** (`unmapped_identity`, 403) — fail closed. Approval and gated-action buttons are intercepted hub-side exactly as on Discord and never reach an agent; a modal-bearing button returns its spec for the client to fill rather than firing. Off ⇒ no store is opened, no card event is published, both routes 503, and `features.cards` is false — byte-identical to before. Discord delivery is untouched in either state. Hub-wide, no per-user canary. |
