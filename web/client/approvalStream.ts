import type { ApprovalOperationsEvent, ConnectionState } from "./types"
...
interface ApprovalStreamDependencies {
...
export class ApprovalStream {
  ...
  private handlers: ApprovalStreamHandlers | null = null
  ...
  constructor(private readonly dependencies: ApprovalStreamDependencies = {
  ...
  private connect(generation: number): void {
    ...
    try {
      ...
      const source = this.dependencies.open(`/api/operations/approvals/events?after=${this.cursor}`, {
        open: () => {
  ...
  private receive(data: string, generation: number, sourceAttempt: number): void {
    if (!this.isCurrent(generation) || sourceAttempt !== this.sourceAttempt) return
    try {
      const event = JSON.parse(data) as ApprovalOperationsEvent
      if (event.kind === "snapshot_required" && validSequence(event.sequence)) {
        this.cursor = event.sequence
        this.handlers!.onInvalidate()
        return
      }
      if (!validSequence(event.sequence) || event.sequence <= this.cursor) return
      this.cursor = event.sequence
