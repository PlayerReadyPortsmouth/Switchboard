...
- **`schedules[]`** — `{ id, cron?, hourUtc?, agent, channelId, message }`. Delivers `message` to `agent`@`channelId` on a 5-field **cron** expression (`min hour dom mon dow`, UTC; supports `*`, lists, ranges, `*/n` steps, and `0`/`7` for Sunday), checked every 30s and deduped per minute per `id`. `hourUtc` remains a daily shorthand (`0 <hourUtc> * * *`). E.g. `"0 * * * *"` hourly, `"*/15 * * * *"` every 15 min, `"0 9 * * 1-5"` 09:00 on weekdays, `"30 9 * * 1"` 09:30 Mondays.
