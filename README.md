...
With `discord.enabled: false`, startup does not read a Discord token or construct a Discord client; durable web conversations, real agent-backed text turns, and the Agents workspace continue to work. Agent operations are shared application services used by the web and Discord compatibility paths and are reusable by future Slack, Teams, or other adapters. Canonical web attachments, consultations, delegations, handoff, Approvals, Operations, and Settings remain pending. See [`docs/architecture/conversations.md`](docs/architecture/conversations.md) and the [`standalone web-client and transport architecture`](docs/superpowers/specs/2026-07-12-standalone-web-client-and-transport-architecture-design.md).
