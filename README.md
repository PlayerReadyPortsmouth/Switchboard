...
- **Replay / time-travel** (`replay.ts`): `!replay <chat|corr> [n]` reconstructs a conversation's (or one action's) effect-chain from the `corr`-threaded ledger — an ordered, grouped timeline. Pure projection over the audit log, operator-gated.
